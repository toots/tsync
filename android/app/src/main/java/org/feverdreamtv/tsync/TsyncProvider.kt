...
class TsyncProvider : DocumentsProvider() {
    ...
    override fun queryChildDocuments(
        ...
        try {
            ...
            response.optJSONArray("dirs")?.let { dirs ->
                for (i in 0 until dirs.length()) {
                    val entry = dirs.getJSONObject(i)
                    addDirectory(
                        cursor, entry.getString("key"),
                        (entry.optDouble("mtime", 0.0) * 1000).toLong()
                    )
                }
            }
            response.optJSONArray("files")?.let { files ->
                for (i in 0 until files.length()) {
                    val entry = files.getJSONObject(i)
                    addFile(
                        cursor, entry.getString("key"), entry.optLong("size"),
                        (entry.optDouble("mtime") * 1000).toLong()
                    )
                }
            }
        } catch (e: Exception) {
    ...
    override fun openDocument(
        ...
        // Whole-file, as the macOS File Provider does: ensure_cached materialises
        // the file and hands back a path we can open directly. Ranged reads via
        // openProxyFileDescriptor are a later milestone; nothing here needs them
        // until something tries to stream rather than open.
        if (!mode.contains("w")) {
            val localPath = Ipc.send(socket, "ensure_cached", mapOf("path" to documentId))
                .getString("localPath")
            return ParcelFileDescriptor.open(File(localPath), ParcelFileDescriptor.MODE_READ_ONLY)
        }

        val staging = File(context!!.filesDir, "staging").apply { mkdirs() }
            .resolve(UUID.randomUUID().toString())
        if (mode.contains("r")) {
            // Edit in place: start from the current contents. The handoff file
            // already lives under cache_root, the same filesystem as the chunk
            // store, so the daemon's rename into it costs nothing.
            runCatching {
                val localPath = Ipc.send(socket, "ensure_cached", mapOf("path" to documentId))
                    .getString("localPath")
                File(localPath).renameTo(staging)
            }
        }

(** The event loop's half of the IPC.

    {!Ipc} is the same socket spoken synchronously, for a caller with no loop to
    block; everything here needs one. Re-exported so a caller that does both
    names one module. *)

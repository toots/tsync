(* The event loop's half of the IPC: the same socket, spoken without blocking
   the loop, and the server that answers on it. *)

include Ipc

let send_lwt ?(timeout = 2.) ~socket_path cmd =
  let open Lwt.Syntax in
  Io_lwt.Clock.with_timeout timeout (fun () ->

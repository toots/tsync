(** What may be written to a link right now, at a rate.
    ...
    Pure: every entry point is handed the time, which is what lets a test read
    a decision without waiting for it. *)
...
(** The fraction of {!stall_timeout} the in-flight window fills. The rest is
    for a rate misjudged and for the store's own time with a body. *)
...
(** Bytes the bucket holds; negative while a body larger than it is being
    paid off. *)
...
(** Seconds until refill alone would admit [bytes]: [0.] now, [infinity]
    when only a {!release} could make room. *)

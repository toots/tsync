...
(** Positioned variants for callers holding their own long-lived file
    descriptor, opened and closed by the caller. No open/seek/close per call,
    and safe under concurrent use of the same [fd] at different offsets:
    pread/pwrite don't touch a shared file position the way lseek+read/write
    would. *)

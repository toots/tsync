...
(* Nothing here may touch Lwt before the forks. The hazard is a child inheriting
   the notification eventfd and having its worker wakeups delivered to the
   parent; [Lwt_unix.fork] reinitialises the notification system in the child,
   which is why {!Frontend.run_forked} uses it. What runs after the forks belongs
   to a leaf, which starts its own loop. *)
        ...
        match List.assoc_opt "action" obj with
          ...
          | Some (`String "uplink") -> (
              ...
              (* A report per link under "links"; a request with the fields
                 ...
                 default link, and is answered with its grant at the top
                 too. *)
              let reports =
                match List.assoc_opt "links" obj with
                  | Some (`Assoc links) ->
                      List.filter_map
                        (fun (name, r) ->
                          match r with
              ...
              match
                Uplink_lwt.lease_renewal (Uplink_lwt.process ()) ~pid reports
              with

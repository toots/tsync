(** Drop the references that keep old content alive: version history, trashed
    ...
    separate step, and the two are separate commands because only some stores can
    do the second.
...
module Make (C : Conf.S) : sig
  (** [expire ~cutoff ()] empties trashed folders, then deletes versions and
      ...
      listing one, [on_scan] once its size is known, and [on_delete] per batch of
      deletions with the running total, listing being the part that cannot report

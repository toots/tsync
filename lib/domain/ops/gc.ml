...
struct
  ...
  module Make (C : Conf.S with type 'a io = 'a Io.t) = struct
    ...
    let collector () =
      ...
      match main with
        | None ->
            Io.fail
              (Unsupported
                 (Printf.sprintf "%s has no main store to collect."
                    C.domain_name))
        | Some m -> (
            let (module M : C.Store) = m.Backend.backend in
            (* A collection is renames within the store's own tree, so it needs a
               store that is one. *)
              match M.local_path with
              | Some root -> Io.return ((module M : C.Store), root)
              | None ->
                  Io.fail
                    (Unsupported
                       (Printf.sprintf
                          "Collecting chunks needs a local main store; %s is \
                           %s. Versions and the journal can still be trimmed \
                           with tsync expire."
    ...
    let busy () =
      ...
      Busy
        (Printf.sprintf
           "A collection of %s is already running%s. Wait for it, or stop it \
            and use tsync gc --abort."
    ...
    let take_lock root =
      if !held then
        ...
        let* taken = Lockfile.take path in
        match taken with
          | Some lock ->
              held := true;
              Io.return lock
    ...
    (* Dropping it is worth doing even though exiting would, since a process
       that collects twice must not go on holding it. *)
    let drop_lock lock =
      held := false;
      Lockfile.drop lock
    ...
    let open_space root =
      ...
      let* exists = Syscalls.file_exists dst in
      if exists then Io.return () (* a previous attempt got this far *)
      else
        Io.catch
          (fun () -> Syscalls.rename src dst)
          (function
    ...
    let namespaces root =
      let read dir = Files.readdir_list_quiet (Filename.concat root dir) in
    ...
    let prefix_of_namespace root ns =
      let base =
        ...
        ^ id
      in
      let+ kind = Files.lstat_kind (Filename.concat root base) in
    ...
    let referenced_chunks key =
      if Stored_key.is_dir_key key then Io.return []
      else
          ...
          | None -> (
              match Manifest.of_string data with
                ...
                | exception e ->
                    failwith
                      (Printf.sprintf
                         "cannot read manifest %s (%s); aborting the \
                          collection with nothing discarded"
    ...
    (* What is left in one shard of the space on its way out, which after a
       ...
       the store's size. *)
    let orphans_in_shard ~main:(module M : C.Store) ~slots shard =
      ...
      let candidates =
        List.filter_map
          (fun (e : Backend.file_entry) ->
            ...
            if Chunks.is_chunk_key name then Some (name, e.Backend.size)
            else None)
          going
      in
      if candidates = [] then Io.return ([], 0)
      else
        let+ doomed =
          Pools.filter_map_with slots
            (fun (name, size) ->
    ...
    type session = {
      ...
      root : string;
      started : float;
      targets : (module C.Store) Backend.member list;
      lock : Lockfile.t;
    ...
    let save s phase cursor =
      Space.write_run { Collection.phase; started = s.started; cursor }
    ...
    let start ?concurrency ?delete_batch ?(keep = false) ?(verify = false) () =
      ...
      (* I/O bound, so a batch wants concurrent syscalls rather than cores: the
         ...
         can be. *)
      let (module M : C.Store) = main in
      let* max_slots =
        match concurrency with
          | Some n -> Io.return (max 1 n)
          | None ->
      ...
      let fresh work total =
        {
          ...
          verify;
          lock;
          unit_slots = Pools.create ~max:max_slots ();
          item_slots = Pools.create ~max:max_slots ();
          ...
          bytes_reclaimed = 0;
          on_mark =
            (fun ~namespaces:_ ~total:_ ~roots:_ ~promoted:_ ~at:_ -> ());
      ...
      List.iter
        (fun (name, count, oldest) ->
          Log.warn
            "gc: %s has %d delete request(s) outstanding, oldest %s — check \
             the bucket's notification and the function's logs"
      ...
      (* Each phase is recorded before the step it names, so whatever state the
        ...
        | Collection.Abandoning ->
            ...
            Log.info "gc: resuming abandonment, %d shard(s) left to keep"
              (List.length pending);
            Io.return (fresh (Keep pending) (List.length pending))
        ...
        | Collection.Closing ->
            ...
            Io.return (fresh (Close pending) (List.length pending))
    ...
    let report_close ?force s =
      ignore
        (throttled ?force ~key:"close" (fun () ->
             Log.debug "gc: %d/%d shard(s) closed, %d chunk(s) reclaimed"
               s.done_ s.total s.chunks_reclaimed;
    ...
    let verify_promoted s ck =
      ...
      let (module M : C.Store) = s.main in
        ...
        | Some marker -> (
            ...
            let* outcome =
              Io.catch
                ...
                (fun exn -> Io.return (`Unreadable (Retry.reason exn)))
            in
            match outcome with
              | `Body body ->
              ...
              | `Unreadable why ->
                  ...
                  Log.err "gc: chunk %s could not be read (%s): filing %s" ck
                    why
                    (Stored_key.to_string marker);
    ...
    let mark_root s key =
      ...
      let* () =
        iter_p
          (fun ck ->
            Pools.use s.item_slots (fun () ->
                let* moved = Space.promote ck in
                s.chunks_promoted <- s.chunks_promoted + 1;
                report_mark s;
                if s.verify && moved then verify_promoted s ck else Io.return ()))
          chunks
      in
    ...
    let mark_one s ns =
      ...
      in
      let* () =
        iter_p (fun k -> Pools.use s.unit_slots (fun () -> mark_root s k)) keys
      in
      s.done_ <- s.done_ + 1;
    ...
    (* Not {!Files.rm_rf}, which lstats every name before unlinking it and walks
    ...
    let discard_shard s shard =
      ...
      let* () =
        iter_p
          (fun n ->
            Pools.use s.item_slots (fun () ->
                Files.unlink_quiet (Filename.concat dir n)))
          names
      in
      Io.catch
        (fun () -> Syscalls.rmdir dir)
    ...
    let flush_close s ~shards ~doomed ~reclaimed ~bytes =
      ...
      (* A marker accuses a chunk by naming it ({!Corruption}); once the chunk is
         ...
         keys are already a success ({!C.Store.delete_multi}). *)
      let markers = List.filter_map Chunk_layout.marker_key doomed in
      let* () =
        if markers = [] then Io.return ()
        else
          iter_s
            (fun (module T : C.Store) -> T.delete_multi markers)
            (s.main :: direct)
      in
      let* () = iter_s (discard_shard s) shards in
            ...
            ignore
              (throttled ~key:"close" (fun () ->
                   ...
                   s.on_close ~shards:scanned
                     ...
                     ~at:s.at));
            let* () = if ready () then flush () else Io.return () in
            go more
        | remaining ->
            let+ () = flush () in
            remaining
    ...
    let carry_over s shard =
      ...
      Io.catch
        (fun () ->
          ...
          let* names = Files.readdir_list src in
        ...
        (function
          ...
          | Unix.Unix_error (Unix.ENOENT, _, _) -> Io.return (Some 0)
          | exn -> Io.fail exn)
        ...
        iter_s
          (fun n ->
            if Hashtbl.mem below n then
              Files.unlink_quiet (Filename.concat dst_dir n)
    ...
    let move_across s shard =
      ...
      let+ () =
        if missing = [] then Io.return ()
        else
          let* () = Files.mkdir_p dst_dir in
          iter_p
            (fun n ->
              Pools.use s.item_slots (fun () -> move_into ~src_dir ~dst_dir [n]))
            missing
      in
      List.length names
    ...
    let discard_from_space s = Files.rm_rf (from_dir s.root)
    ...
    let run ?budget ?(units = 256) ?pause ?concurrency ?delete_batch
          ...
          | `More ->
              if deadline () then begin
                ...
                let* () =
                  match pause with
                    | None -> Io.return ()
                    | Some seconds -> Clock.sleep seconds
                in
                loop ()
      in
      (* Released whichever way this ends, including on the exception a failed
         ...
         the process is not. *)
      Io.finalize loop (fun () -> release s)

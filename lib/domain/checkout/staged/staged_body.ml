...
struct
  ...
  module Make (C : Conf.S with type 'a io = 'a Io.t) (Cache : Cache) = struct
    ...
    let len_of ~uuid =
      Io.catch
        (fun () ->
          let+ st = Retry.LargeFile.stat (path uuid) in
    ...
    let ensure ~uuid ~len =
      ...
      match have with
        | Some n when n >= len -> return_unit
        | _ ->
            let* fd = Retry.openfile p [Unix.O_WRONLY; Unix.O_CREAT] 0o644 in
            Io.finalize
              (fun () -> Retry.LargeFile.ftruncate fd (Int64.of_int len))
              (fun () -> Retry.close fd)

    let resize ~uuid ~len =
      let* fd = Retry.openfile (path uuid) [Unix.O_WRONLY] 0o644 in
      Io.finalize
        (fun () -> Retry.LargeFile.ftruncate fd (Int64.of_int len))
        (fun () -> Retry.close fd)

    let write ~uuid buf ~offset =
      Fs.write (path uuid) buf ~offset:(Int64.of_int offset)

    let read_into ~uuid buf ~offset =
      Fs.read (path uuid) buf ~offset:(Int64.of_int offset)
    ...
    let forget ~uuid = Fs.unlink_quiet (path uuid)
    ...
    let link_group ~uuid ~len ~group =
      if len <> Manifest.Group.bytes group then return_false
      else
        Io.catch
          (fun () ->
    ...
    let adopt_whole ~src ~uuid =
      ...
      Io.catch
        (fun () -> Retry.rename src dst)
        (function
          | Unix.Unix_error (Unix.EXDEV, _, _) ->
              let* () = Fs.copy_file ~src ~dst in
              Fs.unlink_quiet src
          | exn -> Io.fail exn)

    let whole_read_into ~uuid buf ~offset =
      Fs.read (whole_path uuid) buf ~offset

    let whole_forget ~uuid = Fs.unlink_quiet (whole_path uuid)
  end
end

...
module type S = sig
  ...
  (** [key]'s published manifest, read from the local mirror, which holds one
      sidecar per key the poller has replayed and is the whole answer: a name it
      does not carry is a name the domain does not have.
  ...
  *)
  ...
  (** Files currently waiting on the network because something is reading them —
      ...
      Synchronous on purpose: the table is mutated by reads on the same loop, so
      a fold that could yield would see it change underneath itself.

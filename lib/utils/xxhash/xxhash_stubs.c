...
/* The bigstring variants exist so a chunk body can be hashed where it lies:
   materialising one as a string to name it would allocate the very megabytes the
   bigstring is there to keep off the heap. */

CAMLprim value caml_xxh3_64_bigstring_with_seed(value _data, value _seed)
{
    CAMLparam2(_data, _seed);
    XXH64_hash_t hash = XXH3_64bits_withSeed(

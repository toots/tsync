...
let key t i =
  if i < 0 || i >= t.count then
    invalid_arg (Printf.sprintf "Manifest.key: %d of %d" i t.count);
...
let builder ~name ~size ~chunk_size ~mtime ~symlink ~count =
  if count < 0 then
    invalid_arg (Printf.sprintf "Manifest.builder: count %d" count);
...
let key_offset b i =
  if i < 0 || i >= b.b_count then
    invalid_arg (Printf.sprintf "Manifest.builder: %d of %d" i b.b_count);
  b.b_keys_at + (i * key_bytes)

let set b i key =
  let off = key_offset b i in
  if String.length key <> key_bytes then
    invalid_arg (Printf.sprintf "Manifest.set: chunk key %S" key);
...
let seal b ~h1 ~h2 =
  let fixed_hex what s =
    if String.length s <> 16 then
      invalid_arg (Printf.sprintf "Manifest.seal: %s is %S" what s)
  in
  fixed_hex "h1" h1;
  fixed_hex "h2" h2;
...
(* Hashed over the ordered chunk digests, so a changed file's manifest rebuilds
   from its chunk keys without re-reading untouched bytes. *)
...
let to_string ~name (m : t) =
  encode ~name ~size:(size m) ~chunk_size:(chunk_size m) ~mtime:(mtime m)
    ~h1:(h1 m) ~h2:(h2 m) ~symlink:(symlink m)
    ~keys:(List.init (count m) (key m))
...
module Group = struct
  (* Bound before this module's own [key] and [count] shadow them. *)
  let chunk_count = count
  let chunk_key_at = key
  let chunk_len = len
  ...
  (* [None] only when [i] is out of range: {!Manifest} rejects at decode any
     body whose length disagrees with its header, so every index below [count] has
     a key. *)
  let of_table ~table ~per i =
    let n = chunk_count table in
    if per <= 0 || i < 0 || i >= n then None
    else (
      let first = i / per * per in
      let len = min per (n - first) in
      let members = Array.init len (fun j -> chunk_key_at table (first + j)) in
      Some
        {
          key = key_of members;
          members;
          sizes = Array.init len (fun j -> chunk_len table (first + j));
          first;
        })
    ...
    if per <= 0 then []
      ...
      let rec go i acc =
        if i >= n then List.rev acc
          ...
          go (i + per)
            (match of_table ~table ~per i with
              | Some g -> g :: acc
              | None -> acc)
      in
      go 0 [])
  ...
  let count ~table ~per =
    if per <= 0 then 0 else (chunk_count table + per - 1) / per

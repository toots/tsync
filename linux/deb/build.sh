...
#
# setup.sh, generated by linux/repo/build.sh, spells this rule again to find the
# repository directory from a client's own os-release; changing it in one place
# alone points installs at a directory nothing publishes.
. /etc/os-release
case "$ID" in

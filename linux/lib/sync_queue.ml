...
let exec_delete t key entry_key ops =
  ...
  with exn -> Log.err "sync_queue delete %s: %s" key (Printexc.to_string exn)
...
let exec_mkdir t key entry_key ops =
  try
    ...
    ignore (File_store.write_journal_entry ~entry_key ops t.store);
    t.on_version ~entry_key
  with exn -> Log.err "sync_queue mkdir %s: %s" key (Printexc.to_string exn)
...
let exec_rmdir t key entry_key ops =
  try
    ...
    ignore (File_store.write_journal_entry ~entry_key ops t.store);
    t.on_version ~entry_key
  with exn -> Log.err "sync_queue rmdir %s: %s" key (Printexc.to_string exn)

let exec_rename t ~src_key ~dst_key ~src_is_dir entry_key ops =
  try
    if src_is_dir then
      File_store.rename_directory t.store ~src_prefix:src_key
        ~dst_prefix:dst_key
    else File_store.rename_file t.store ~src_key ~dst_key;
    ignore (File_store.write_journal_entry ~entry_key ops t.store);
    t.on_version ~entry_key
  with exn ->
    Log.err "sync_queue rename %s->%s: %s" src_key dst_key
...
let post t event =
  match event with
    ...
    | Mkdir { key; entry_key; ops } -> exec_mkdir t key entry_key ops
    | Rmdir { key; entry_key; ops } -> exec_rmdir t key entry_key ops
    | Evict { key } -> (
        ignore (cancel_put t key);
        try
          ...
          t.on_evict ~key
        with exn ->
          Log.err "sync_queue evict %s: %s" key (Printexc.to_string exn))
    | Rename
        {
          src_key;
          dst_key;

...
| Frontend type | `fuse` on Linux, `file_provider` on macOS — both offered as the default. `android` is for a domain the Android app drives one command at a time, with nothing to mount. |

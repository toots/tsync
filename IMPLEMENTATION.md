...
**Responses:** `{"ok":true, ...}` with action-specific fields (e.g. `size`, `mtime`, `etag`, `isUploaded`, `localPath`, `dirs`, `files`), or `{"ok":false,"error":"<message>"}` on failure. The listed backend objects are manifests, so `list_dir`/`list_all` read each file's manifest to report the **logical** `size`/`mtime` and the content hash (`h1`) as `etag` — the same identity `stat` returns (empty for dirty files). `list_dir` returns directories as objects `{"key":"<full-key-ending-in-/>","mtime":<float>}` (mtime present when a directory marker object exists in the backend; absent for implicit directories inferred from their children's keys).

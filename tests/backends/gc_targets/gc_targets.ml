...
let () =
  ...
  Lwt_main.run
    ((* Chunks 1 and 3 are live: one manifest names both. Chunk 2 is on the main
     ...
     let* () = Replica.put ~key:(key 1) ~data:(Bigstring.of_string "live-one") () in
     let* () = Replica.put ~key:(key 2) ~data:(Bigstring.of_string "orphaned") () in
     let* () =
     ...
     let* () = Main.put ~key:(key 4) ~data:(Bigstring.of_string "orphan-4") () in
     let* () = Replica.put ~key:(key 4) ~data:(Bigstring.of_string "orphan-4") () in
     let* s = G.start () in
     let rec until phase =

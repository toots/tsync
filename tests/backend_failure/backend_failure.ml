(* Every backend maps its own errors into {!Backend.kind}, and the shared retry
...
let show name exn =
  Printf.printf "%-28s %s\n" name
    (Backend.string_of_kind (Backend.classify exn))

let () =
  print_endline "classify";
  show "s3 503 (transient)"
    (Backend.failed ~kind:Backend.Transient ~op:"put" "HTTP 503");
  show "s3 403 (permanent)"
    (Backend.failed ~kind:Backend.Permanent ~op:"put" "forbidden");
...
let () =
  ...
  attempts "permanent"
    (Backend.failed ~kind:Backend.Permanent ~op:"put" "forbidden");
  attempts "cancelled" Backend.Cancelled;

...
module Src = (val Fixture.local_store src_dir)
module Real = (val Fixture.local_store dst_dir)
...
module C =
  (val Fixture.conf ~max_chunk_buffers:2
         ~store:
           (Domain_store_lwt.make
              ~mains:
                [{ Domain_store_lwt.name = "source"; backend = (module Src) }]
              ~targets:[]
              ~archives:

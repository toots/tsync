...
let () =
  ...
  check "full at birth: two seconds of rate" (Uplink_budget.tokens b ~now:0. = 2000.);
  ...
  check "a second later, a second's worth" (Uplink_budget.tokens b ~now:1. = 1000.);
  ...
  check "and is refused until then" (not (Uplink_budget.admits b ~now:3. ~bytes:1));
  ...
  check "what the old rate earned is kept" (Uplink_budget.tokens b ~now:1. = 1000.);

...
module C =
  (val let store = (module Shareable : Backend_lwt.Store) in
       Fixture.conf
         ~chunk_size:(8 * 1024 * 1024)
         ~cache_chunk_size:(8 * 1024 * 1024)
         ~store
         ~members:[Backend.member ~name:"local" store]
         ~root ()
      : Conf_lwt.S)
...
let () =
  ...
  Lwt_main.run
    (let open Lwt.Syntax in
     ...
     let* file_key = Lwt.map Option.get (L.manifest_key (Lk.file "foo")) in
     let* () =

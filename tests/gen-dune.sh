...
deps_for() {
  case "$1" in
    frontends/android | frontends/android_bridge | frontends/android_lazy | unit/completion | unit/export_cli | unit/ls_listing)
...
for dune in $(find . -mindepth 2 -name dune | sed 's|^\./||' | sort); do
  ...
  case "$dir" in
    e2e/linux | e2e/macos | e2e/stress | conformance | live/*) continue ;;
  esac

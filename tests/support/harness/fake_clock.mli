(** A clock a test turns by hand.
    ...
    instant out. The first such double in the tree; the modules it stands in for
    are functors over the signature, so nothing changes to accept it.
...
(** Move the clock forward by [seconds] and wake every sleep whose deadline has
    passed, earliest first. The wakers run on the next scheduler turn, so a test
    pauses ({!Lwt.pause}) once or twice before reading what they did. *)
val advance : float -> unit

(** Sleeps still waiting for their deadline. A sleep cancelled by [with_timeout]
    or [pick] is no longer counted. *)
